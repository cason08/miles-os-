...
| Framework | Next.js (App Router) | Frontend + backend in one project (pages, API routes, server actions). Enormous ecosystem and AI familiarity. Deploys with zero config to Vercel. Natural path to PWA later (manifest + service worker) and to React Native later (shared types/logic) without a rewrite. |
...
| DB hosting | Neon (or Supabase) | Managed Postgres with automated backups/point-in-time recovery (covers the backup assumption in PRODUCT.md §4.2) and a generous free/cheap tier for one user. Neon's branching is convenient for local/preview environments. |
...
Two requirements demand a real interface boundary, not just a folder name: the email-ingestion layer (PRODUCT.md §3.1) and the reward engine (§3.4). Both are designed the same way — a stable internal contract with one concrete implementation today.
...
// extraction, categorisation, budgets, rewards, or the dashboard.
```
...
```
Every card owns a list of these, stored as JSONB and edited through a settings UI — adding, removing, or editing a card never requires a code change (§3.4). A single generic evaluator function matches a transaction against a card's rules and returns both the computed earn amount *and* the matched rule's `description`, which is what makes every recommendation self-explaining "for free" rather than needing separately-authored copy.
...
- **Account** — bank/card/rewards/airline; institution, type, currency, opening balance, current balance (derived), `isLiability`.
- **Transaction** — accountId, date, amount, currency, sgdAmount, fxRate, `fxEstimated`, merchant (raw + normalised), categoryId, `source` (`gmail_import` | `manual`), `isForeignCurrency`, duplicate-flag fields.
- **RawEmailExtraction** — the untouched AI output for a transaction (gmailMessageId, raw JSON, model used, timestamp) — this is what makes "preserve the original data" (§3.10) a clean separate record rather than shadow columns bolted onto `Transaction`.
- **TransactionAuditLog** — append-only log of every change to a transaction (field, old value, new value, changed-by-system-or-user, timestamp) — the audit trail required by §3.10.
- **Category** — name, monthly budget limit (nullable = no budget set).
- **MerchantCategoryRule** — normalised merchant → category, the "learned correction" memory from §3.3.
- **CreditCard** — belongs to an Account; cycle type (calendar/statement) and cycle start day; holds a list of `RewardRule` (JSONB, §3 above).
- **RewardProgram** / **RewardBalance** — the 5 day-one programmes and current balances (with manual-override timestamps).
- **RewardTransfer** — from-program, to-program, date, ratio, source amount, resulting amount — the permanent transfer-history ledger from §3.5.
- **NotificationItem** — type, title, message, recommended action, reason, related entity, read state — backs the activity centre (§3.8).
- **InstitutionProfile** — sender-address patterns and display metadata per supported bank — the configuration that makes adding a bank mostly data entry, not new parsing code.
- **GmailSyncState** — last historyId / cursor, last synced timestamp, last error — powers both the sync job and the "connection needs re-authorising" health check (see §7 risk).
...
- **Self-imposed narrowing:** even though the granted scope is technically broad, the app never lists or reads mail outside senders matching a configured `InstitutionProfile` — it queries Gmail with sender/subject filters (`from:` a known bank address) rather than scanning the whole mailbox. This is documented explicitly as the least-privilege posture referenced in PRODUCT.md §3.7, since OAuth scopes alone can't express "only these senders."
...
8. Reward engine runs against the new transaction to update bonus-cap usage and miles earned (computed on read, per PRODUCT.md §4.2 assumption — no separate write-time counters to keep in sync).
...
- Backups: rely on Neon's automated point-in-time recovery rather than custom tooling (§4.2 assumption in PRODUCT.md).
...
│   ├── (dashboard)/             # main authenticated app: overview, transactions, budgets, cards, miles
...
│   │   ├── extraction/           # AI client interface + prompt templates + institution profiles
...
- **Local setup:** clone → `pnpm install` → copy `.env.example` to `.env.local` and fill in secrets (Google OAuth client, Anthropic key, database URL) → `pnpm prisma migrate dev` → `pnpm dev`. This sequence is exactly what the "clone and follow a simple setup guide" requirement (PRODUCT.md §3.6) needs to satisfy, and becomes the README.
